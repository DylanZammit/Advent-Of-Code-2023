...
ftop2bot = {i: v.copy() if len(v) else {-1} for i, v in top2bot.items()}
not_disintegrated = set(v.pop() for v in top2bot.values() if len(v) == 1)


def disintegrate(falls):
    prev, curr = 0, len(falls)
    while prev != curr:
        prev = curr
        for i, supp in ftop2bot.items():
            nsupp = supp - falls
            if len(nsupp) == 0:
                falls.add(i)
        curr = len(falls)
...
print(sum(disintegrate({b}) for b in not_disintegrated))
